"""Create one Practice Set A test with its eleven empty sections.

Section titles come from sections.json in the test's data directory, so adding a
test means adding data, not editing this file:
...
Titles are optional; a section without one keeps whatever title it has. Audio
URLs for the four listening parts are always assigned, since every test in the
book has its recording split the same way.

Safe to re-run: existing sections are kept and only their titles are refreshed.

Usage:
    cd backend
    .\\venv\\Scripts\\python scripts\\seed_practice_a_bootstrap.py 2
"""
