"""Seed Practice Set B Test 4 Writing.

Task 1: maps — fishing village (1974) → European tourist resort (2004).
Task 2: music benefits vs negative influence — discuss both + opinion.

Wording is taken from the printed paper (pp.106–107). The teaching
'strategy' boxes around those tasks are not part of the exam and are omitted.

Usage:
    cd backend
    .\\venv\\Scripts\\python scripts\\seed_practice_b_t4_writing.py
"""
...
TASK2_STATEMENT = (
    ...
    "Some people think that music brings only benefits to individuals and "
    "societies. Others, however, think that music can have a negative "
    "influence on both."
)
...
async def seed(db: AsyncSession) -> None:
    ...
    await db.commit()
    print(f"  Task 1  maps {CHART_IMAGE_URL.format(test=TEST_NUMBER)}  min 150 words")
