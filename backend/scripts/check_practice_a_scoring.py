...
this drives the real scoring service over the seeded rows as three candidates:
  ...
  perfect  — the first accepted variant of every answer; must score 40/40.
  messy    — the same answers in lower case with padding; must also score 40/40,
             because IELTS marking ignores case and stray spaces.
  clueless — a string that matches nothing; must score 0/40.

Writing and Speaking are graded by the model, not by keys, so they are skipped.

Nothing is written to the database: answers are built as detached objects.
...
def canonical(question: Question) -> object:
    """The answer a candidate who knows the material would type."""
...
def messy(value: object) -> object:
    """Same answer, typed by someone in a hurry."""
...
def variant_check(questions: list[Question]) -> list[str]:
    """Try each accepted variant on its own and report any the marker refuses."""
    rejected: list[str] = []
    for question in questions:
        key = question.answer_key or {}
        correct = key.get("correct")
...
async def check(db: AsyncSession, test_number: int) -> int:
    ...
    for section_type, to_band in (
        ...
        # Every variant listed as accepted has to actually be accepted. A variant
        # longer than the gap's word limit is silently rejected, which would mark
