...
import type { ExaminerLoadingStage } from './components/examiner-loading-screen'
...
export type SpeakingSessionControls = { stop: () => void }

type SpeakingExaminerSessionProps = {
  attemptId?: string | null
  mode: 'embedded' | 'standalone'
  /** Auto-start the AI session once idle (student take-test flow). */
  autoStart?: boolean
...
export function SpeakingExaminerSession({
  attemptId,
  mode,
  autoStart = false,
  onComplete,
  ...
  const reconnectSimli = useCallback(
    async (options?: { silent?: boolean; remount?: boolean }) => {
      try {
  ...
  const handleSimliConnectionLost = useCallback(() => {
  ...
  const handleStart = useCallback(async (): Promise<boolean> => {
    ...
    // Already in a live session (e.g. retry after a UI glitch) — treat as ok.
    if (
    ...
    if (micStatus !== 'ok') {
      // Silent: Start / autostart should not toast "Microphone is working".
      const micOk = await checkMicrophone({ silent: true })
      if (!micOk) {
        toast.error(
    ...
    try {
      ...
      // Backend returned 200 — always commit. Do not drop the turn because
      // Simli init / Strict Mode flipped phaseRef back to idle mid-request.
      if (signal.aborted && !isSessionActive(sessionId)) {
        setPhase('idle')
        return false
      }
  ...
  const { autoStartPending, blockedReason, retry } = useSpeakingAutostart({
    enabled: autoStart,
    phase,
    isTokenPending,
    score,
  ...
  // Expose stop() to parent via controlsRef
  useImperativeHandle(
    controlsRef,
    () => ({ stop: () => stopSession({ notify: false }) }),
    [stopSession],
  )
  ...
  const bootstrapping =
    ...
    (phase === 'loading' ||
      ...
      autoStartPending ||
      phase === 'thinking' ||
      (autoStart && blockedReason !== null))
  const showLoading = bootstrapping
  const canStart = !autoStart && phase === 'idle' && !score && !isTokenPending
  ...
  const loadingStage: ExaminerLoadingStage =
    blockedReason || autoStartPending || phase === 'thinking'
      ? 'starting'
      : simliEnabled && !simliReady
        ? 'video'
        : 'examiner'

  const loadingAction =
    autoStart && blockedReason ? (
      <div className='flex w-full flex-col items-center gap-3 text-center'>
        <p className='text-lg font-semibold text-foreground'>Tap to begin</p>
        <p className='text-sm text-muted-foreground'>
          {blockedReason === 'mic'
            ? 'Microphone access is required. Allow the microphone in your browser, then tap below.'
            : blockedReason === 'failed'
              ? 'Could not start the speaking test automatically. Tap below to try again.'
              : 'Your browser blocked autoplay. Tap below to start the speaking test.'}
        </p>
        <Button
          size='lg'
  ...
  useEffect(() => {
    onActiveChange?.(isActiveSession)
  }, [isActiveSession, onActiveChange])

  // Notify parent when scoring completes
  const prevPhaseRef = useRef(phase)
  useEffect(() => {
    if (prevPhaseRef.current !== 'done' && phase === 'done') {
      onComplete?.(score?.overall_band ?? null)
    }
    prevPhaseRef.current = phase
  }, [phase, score, onComplete])

  // Space hotkey only in standalone mode
  useEffect(() => {
  ...
  }, [mode, phaseRef, startRecording, stopRecording, cancelBrowserSpeech])

  const showUserCamera = phase !== 'done'
  const showSimliAvatar = simliEnabled && simliToken && simliFaceId && phase !== 'done'
  ...
  const stage = (showSimliAvatar || showUserCamera) && (
    <VideoStage
      ...
      showStatus={phase !== 'idle' && phase !== 'loading' && !showLoading}
  ...
  return (
    <div
      className={cn(
        'flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto p-2 sm:p-3 md:gap-5',
        isActiveSession && 'overflow-hidden px-3 py-2 sm:px-4 sm:py-3',
      )}
    >
      {endTestDialog}
      {sessionShell}
